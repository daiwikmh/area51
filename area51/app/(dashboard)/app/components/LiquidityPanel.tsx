...
export default function LiquidityPanel({ state: _state, onAction }: Props) {
  ...
  async function addLiquidity() {
    ...
    try {
      ...
      await onAction("addLiquidity", { amount0: a0.toString(), amount1: a1.toString() });
  ...
  async function requestWithdrawal() {
    ...
    try {
      ...
      setStatus("withdrawal requested — check keeper for ratio");
    } catch (e) {
      setStatus(e instanceof Error ? e.message : "error");
    } finally {
      setBusy(false);
  ...
  return (
    <div className="control-card">
      <div className="field-label mb-3">LIQUIDITY</div>
      ...
      {tab === "add" ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            Keeper will compute plaintext ratio from sealed shares before finalising.
          </p>
          <button
